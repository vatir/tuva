...
def func(param, return_oofunvalue=False):
    ...
    lorentz1 = (param['signal1']*param['width1']**2.0)/((param['x']-param['position1'])**2.0+param['width1']**2.0)

    position2=param['position1']+param['delta1']
    
    lorentz2 = (param['signal2']*param['width2']**2.0)/((param['x']-position2)**2.0+param['width2']**2.0)
 
    position3 = position2+param['delta2']
    
    lorentz3 = (param['signal3']*param['width3']**2.0)/((param['x']-position3)**2.0+param['width3']**2.0)
    
    position4 = position3+param['delta3']
    
    lorentz4 = (param['signal4']*param['width4']**2.0)/((param['x']-position4)**2.0+param['width4']**2.0)
 
    position5 = position4+param['delta4']
    
    lorentz5 = (param['signal5']*param['width5']**2.0)/((param['x']-position5)**2.0+param['width5']**2.0)
    
    position6 = position5+param['delta5']
    
    lorentz6 = (param['signal6']*param['width6']**2.0)/((param['x']-position6)**2.0+param['width6']**2.0)
    
        
    return lorentz1+lorentz2+lorentz3+lorentz4+lorentz5+lorentz6+param['slope']*param['x']